...
export const PostSchema = z.object({
  title: z.string().min(3, ValidationMessages.TOO_SHORT(3)).max(60, ValidationMessages.TOO_LONG(60)),
  smallDescription: z.string().min(10, ValidationMessages.TOO_SHORT(10)).max(160, ValidationMessages.TOO_LONG(160)),
  articleContent: z.union([z.string().min(1, ValidationMessages.EMPTY_CONTENT), z.object({}).passthrough()]),
 ...
 */
const validateSlugUniqueness = (options?: { isSlugUnique?: () => Promise<boolean>; currentPostId?: string }) => {
 ...
 */
export function PostCreationSchema(options?: { isSlugUnique?: () => Promise<boolean> }) {
  return z.object({
    title: z.string().min(3, ValidationMessages.TOO_SHORT(3)).max(60, ValidationMessages.TOO_LONG(60)),
    smallDescription: z.string().min(10, ValidationMessages.TOO_SHORT(10)).max(160, ValidationMessages.TOO_LONG(160)),
    articleContent: z.union([z.string().min(1, ValidationMessages.EMPTY_CONTENT), z.object({}).passthrough()]),
    slug: CommonFields.slug().superRefine(validateSlugUniqueness(options)),
    postCoverImage: z.string().nullable().optional(),
    contentImages: z.union([z.string(), z.array(z.any()), z.object({}).passthrough()]).optional(),
    keywords: z.string().optional(),
  });
 ...
 */
export function PostEditSchema(options?: { isSlugUnique?: () => Promise<boolean>; currentPostId?: string }) {
  return z.object({
    title: z.string().min(3, ValidationMessages.TOO_SHORT(3)).max(60, ValidationMessages.TOO_LONG(60)),
    smallDescription: z.string().min(10, ValidationMessages.TOO_SHORT(10)).max(160, ValidationMessages.TOO_LONG(160)),
    articleContent: z.union([z.string().min(1, ValidationMessages.EMPTY_CONTENT), z.object({}).passthrough()]),
